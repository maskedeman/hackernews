...
func (link Link) Save() int64 {
	stmt, err := mysql.Db.Prepare("INSERT INTO Links(Title,Address,UserID) VALUES(?,?,?)")
	if err != nil {
		log.Fatal(err)
	}
...
func GetAll() []Link {
	stmt, err := mysql.Db.Prepare("select L.id, L.title, L.address,L.UserID,U.Username from Links L inner join Users U on L.UserID=U.ID")
	if err != nil {
		log.Fatal(err)
	}
